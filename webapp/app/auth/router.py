...
from app.auth.schemas import SUserRegister, SUserAuth, UserBase, SUserAddDB, SUserInfo
...
async def register_user(user_data: SUserRegister, session: AsyncSession = TransactionSessionDep) -> dict:
    user = await UsersDAO.find_one_or_none(session=session, filters=UserBase(name=user_data.name))
    if user:
...
async def auth_user(response: Response, user_data: SUserAuth, session: AsyncSession = SessionDep):
    check = await authenticate_user(session=session, username=user_data.name, password=user_data.password)
    if check is None:
...
@router.get("/{user_id}")
async def get_user(user_id: int, session: AsyncSession = SessionDep):
    user_data = await UsersDAO.find_one_or_none_by_id(user_id, session)
    return SUserInfo.model_validate(user_data)


@router.get("/all_users/")
async def get_all_users(session: AsyncSession = SessionDep) -> List[SUserInfo]:
